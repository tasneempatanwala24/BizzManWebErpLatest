...
{
    public partial class wfFaTrialBalance : System.Web.UI.Page
    {
        ...
        public static string FetchMasterDetails(string Id = "")
        {
           // clsMain objMain = new clsMain();
            ...
            try
            {
              ...
              //  dtMaterialList = objMain.dtFetchData(@"select Id, LedgerName, FinancialYearId, Dr_Amount, Cr_Amount
               //                    from vwFaTrialBalance where Id = " + Id + "");


                dtMaterialList = objMain.dtFetchData(@"select LedgerId,LedgerName,VoucherEntryId,VoucherType,EntryType,Amount,TransactionType,Narration
                                   from vwFaVoucherEntryDisplay
                                   where LedgerId = " + Id + "");
          ...
          //  clsMain objMain = new clsMain();
            ...
            try
            {
               // dtEmpList = objMain.dtFetchData(@"select a.Id, a.KpiGroupId, b.KpiGroupName, a.KpiSubGroupName 
                ...
                dtEmpList = objMain.dtFetchData(@"select Id, LedgerName, FinancialYearId, Dr_Amount, Cr_Amount
                                                 from vwFaTrialBalance
                                                ");

                //  SELECT LedgerName, FinancialYearId, Dr_Amount, Cr_Amount
                //FROM vwFaTrialBalance
            }
            catch (Exception ex)
            {

...
function BindMasterList() {
    $.ajax({
        type: "POST",
        url: 'wfSdSalesMisReport.aspx/FetchMasterList',
        data: {},
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        beforeSend: function () {
        ...
        success: function (response) {
            ...
            var html = '';
            for (var i = 0; i < JSON.parse(response.d).length; i++) {
                html = html + '<tr onclick="FetchMasterDetails(\'' + data[i].SalesOrderId + '\')"><td>' + data[i].SalesOrderId + '</td>'
                    ...
                    + '<td>' + data[i].DeliveredQty + '</td></tr>';
                // SalesOrderId,OrderDate,CustomerName,MaterialName,Qty,Tax,UnitPrice,SubTotal,DeliveredQty
            }
...
function FetchMasterDetails(id) {
    $.ajax({
        type: "POST",
        url: 'wfSdSalesMisReport.aspx/FetchMasterDetails',
        data: JSON.stringify({
            "Id": id
        }),
        contentType: "application/json; charset=utf-8",

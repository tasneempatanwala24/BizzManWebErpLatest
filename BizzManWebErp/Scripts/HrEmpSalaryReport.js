...
function BindBranchDropdown() {
    $.ajax({
        type: "POST",
        url: 'wfHrEmpSalaryReport.aspx/BranchMasterList',
        data: {},
...
function BindEmployeeDropdown(brnch) {
    ...
    if (chk == 1) {
        $.ajax({
            type: "POST",
            url: 'wfHrEmpSalaryReport.aspx/EmployeeMasterList',
            data: JSON.stringify({
                "branchid": branch
            }),
            contentType: "application/json; charset=utf-8",
...
function FetchEmployeeDetails(empid) {
    $.ajax({
        type: "POST",
        url: 'wfHrEmpSalaryReport.aspx/FetchEmployeeDetails',
        data: JSON.stringify({
            "EmpId": empid
        }),
        contentType: "application/json; charset=utf-8",
...
function BindEmployeeSaleryApproveList(tbodyid, tbl_id, branchid, month, year, empid, SalaryType, SalaryApprove) {
    $.ajax({
        type: "POST",
        url: 'wfHrEmpSalaryReport.aspx/FetchEmployeSalaryApproveList',
        data: JSON.stringify({
            "branchid": branchid,
            "month": month,
            "year": year,
        ...
        success: function (response) {
            ...
            for (var i = 0; i < data.length; i++) {
                html = html + '<tr>'
                    /*+ '<td><button type="button"  id="previewBtn" onclick="PrintPreview(' + data[i].Id + ')"><span class="fa fa-file" aria-hidden="true"></span></button></td>'*/
                    + '<td><button type="button"  id="previewBtn" onclick="window.open(' + "'" + "wfHrEmpSalaryPaySlip.aspx?id=" + data[i].Id + "','" + "_blank" + "'" + ');"><span class="fa fa-file" aria-hidden="true"></span></button></td>'
                ...
                $('#btnExportPDF').click(function (event) {
                    ...
                    table.$('input[type="checkbox"]').each(function () {
                        ...
                        if (this.checked) {
                            ...
                            $.ajax({
                                type: "POST",
                                url: 'wfHrEmpSalaryReport.aspx/SalaryExportToPDF',
                                data: JSON.stringify({
                                    "id": id
                                }),
                                contentType: "application/json; charset=utf-8",
...
function RejectSalaryApprove(id) {
    $.ajax({
        type: "POST",
        url: 'wfHrEmpSalaryReport.aspx/RejectSalaryApprove',
        data: JSON.stringify({
            "id": id,
...
function UpdateSalaryApprove(id) {
    $.ajax({
        type: "POST",
        url: 'wfHrEmpSalaryReport.aspx/UpdateSalaryApprove',
        data: JSON.stringify({
            "id": id,
            "loginuser": $('#ContentPlaceHolder1_loginuser').val()
        }),
...
function PrintPreview(Id) {
    ...
    $.ajax({
        type: 'POST',
        url: 'wfHrEmpSalaryReport.aspx/GetPdfContent',
        contentType: 'application/json; charset=utf-8',
        data: JSON.stringify({
            "Id": Id
        }),

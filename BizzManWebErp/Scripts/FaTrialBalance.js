...
function BindKpiGroupDropdown() {
    $.ajax({
        type: "POST",
        url: 'wfFaTrialBalance.aspx/KpiGroupList',
        data: {},
...
function BindMasterList() {
    $.ajax({
        type: "POST",
        url: 'wfFaTrialBalance.aspx/FetchMasterList',
        data: {},
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        beforeSend: function () {
...
function FetchMasterDetails(id) {
    $.ajax({
        type: "POST",
        url: 'wfFaTrialBalance.aspx/FetchMasterDetails',
        data: JSON.stringify({
            "Id": id
        }),
        contentType: "application/json; charset=utf-8",
        ...
        //    $('#txtId').attr("readonly", "readonly");
           ...
           // var data = JSON.parse(response.d);
            $('#tblEmpJobList2').DataTable().clear();
            $('#tblEmpJobList2').DataTable().destroy();
            $('#tbody_EmpJob_List2').html('');
            var html = '';
            for (var i = 0; i < JSON.parse(response.d).length; i++) {
                html = html + '<tr <td>' + data[i].LedgerId + '</td>'
                    + '<td>' + data[i].LedgerName + '</td>'
                    + '<td>' + data[i].VoucherEntryId + '</td>'
                    + '<td>' + data[i].VoucherType + '</td></tr>';
            }

            //LedgerId,LedgerName,VoucherEntryId,VoucherType,EntryType,Amount,TransactionType,Narration

            $('#tbody_EmpJob_List2').html(html);
            $('#tblEmpJobList2').DataTable();
...
function AddData() {
    ...
    if (isValid) {
        $.ajax({
            type: "POST",
            url: 'wfFaTrialBalance.aspx/CheckDataAvailability',
            ...
            success: function (response) {
                ...
                if (data == 'False') {
                    $.ajax({
                        type: "POST",
                        url: 'wfFaTrialBalance.aspx/AddData',
                        data: JSON.stringify({
